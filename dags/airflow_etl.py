...
def task_insert_locations():
    ...
    location_table = metadata.tables['location']
    ...
    locations_monkeypox = df_monkeypox[['location', 'iso_code']].drop_duplicates().rename(columns={'location': 'name'})
    ...
    for _, row in locations_all.iterrows():
        ...
        if result is None:
            ins = location_table.insert().values(name=row['name'], iso_code=row['iso_code'])
            session.execute(ins)
    session.commit()
    session.close()
    print("Insertion des locations terminée.")
...
def task_insert_viruses():
    ...
    metadata = MetaData()
    metadata.reflect(bind=engine)
    virus_table = metadata.tables['virus']
...
def task_insert_worldmeter():
    ...
    L'insertion se fait avec ON CONFLICT DO NOTHING afin d'éviter les doublons
    (ici, on suppose qu'une contrainte unique existe sur (date, location_id, virus_id)).
    """
    engine = get_engine()
    Session = sessionmaker(bind=engine)
    session = Session()
    ...
    metadata.reflect(bind=engine)
    location_table = metadata.tables['location']
    worldmeter_table = metadata.tables['worldmeter']
...
with DAG(
    ...
    catchup=False,
    description="DAG ETL amélioré pour éviter les doublons et répartir les sous-parties en tâches"
) as dag:
