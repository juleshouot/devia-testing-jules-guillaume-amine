...
def load_mortality_model():
    ...
    try:
        ...
        model_files = [f for f in os.listdir(MODEL_DIR) if 'mortality' in f and f.endswith('_model.joblib')]
        ...
        feature_cols = metadata.get('feature_cols', [
            'cases_7day', 'deaths_7day', 'cases_7day_lag', 'days_since_start',
            'treatment_improvement', 'healthcare_pressure', 'total_cases',
            'total_deaths', 'new_cases', 'new_deaths', 'new_cases_per_million',
...
def create_features_from_data(df, current_idx):
    ...
    try:
        ...
        data['healthcare_pressure'] = (data['cases_7day'] / data['cases_7day'].max()).fillna(0)
    ...
    for i in range(30, len(df) - 14):  # Besoin d'historique et de données futures
        ...
        if actual_mortality is not None and actual_mortality > 0:
            ...
            results.append({
                ...
                'rel_error': abs(predicted_mortality - actual_mortality) / max(actual_mortality, 0.001),
                # Données de contexte
...
def show_prediction_details(df, location):
    ...
    try:
        ...
        print(f"Meilleure prédiction: {display_df['rel_error'].min() * 100:.1f}% d'erreur")
        ...
        if len(display_df) > 10:
            import numpy as np
            correlation_cases = np.corrcoef(display_df['total_cases'], display_df['rel_error'])[0, 1]
            correlation_deaths = np.corrcoef(display_df['total_deaths'], display_df['rel_error'])[0, 1]
...
def save_prediction_details_csv(df, location, title):
    ...
    try:
        ...
        csv_df['quality'] = csv_df['rel_error'].apply(lambda x:
        ...
        final_df = csv_df[['date', 'predicted_pct', 'actual_pct', 'error_pct', 'rel_error_pct', 'quality',
                           'total_cases', 'total_deaths', 'new_cases_7d', 'new_deaths_7d', 'cases_per_million']]
        final_df.columns = ['Date', 'Taux_Prédit_%', 'Taux_Réel_%', 'Erreur_Absolue_%', 'Erreur_Relative_%', 'Qualité',
                            'Total_Cas', 'Total_Décès', 'Nouveaux_Cas_7j', 'Nouveaux_Décès_7j', 'Cas_Par_Million']
...
def save_validation_plots(results, location):
    ...
    try:
        ...
        axes[0, 1].plot(df['date'], df['rel_error'] * 100, marker='o', markersize=3, alpha=0.7)
        ...
        axes[1, 1].plot(df['date'], df['actual'] * 100, label='Réel', alpha=0.8, linewidth=2)
        axes[1, 1].plot(df['date'], df['predicted'] * 100, label='Prédit', alpha=0.8, linewidth=2)
        ...
        # Sauvegarder
        plot_path = os.path.join(STATIC_DIR, f'validation_mortalite_{location.lower().replace(" ", "_")}.png')
    ...
    if all_results:
        ...
        if location_summaries:
            ...
            for _, row in top_locations.iterrows():
                print(f"  {row['Localisation']}: R² = {row['R²']:.3f}, Erreur = {row['Erreur_Médiane_%']:.1f}%")
