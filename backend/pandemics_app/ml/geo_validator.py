...
def load_geographical_model():
    ...
    try:
        ...
        model_files = [
            ...
            if "geographical_spread" in f and f.endswith("_model.joblib")
        ]
...
def get_geographical_spread_data():
    ...
    try:
        ...
        print(
            f"Période: {first_cases['first_date'].min()} → {first_cases['first_date'].max()}"
        )
...
def prepare_spread_time_series(first_cases_df, virus_name):
    ...
    try:
        ...
        virus_data["yearweek"] = (
            virus_data["year"].astype(str)
            + "-"
            + virus_data["week"].astype(str).str.zfill(2)
        )

        # Compter par semaine le nombre de nouvelles localisations touchées
        spread_data = (
            virus_data.groupby("yearweek").size().reset_index(name="new_locations")
        )
        ...
        date_range = pd.date_range(
            start=spread_data["date"].min(), end=spread_data["date"].max(), freq="W-WED"
        ...
        print(
            f"✅ Série temporelle créée pour {virus_name}: {len(spread_data)} semaines"
        )
        print(f"Total de nouvelles localisations: {spread_data['new_locations'].sum()}")
        print(
            f"Pic de propagation: {spread_data['new_locations'].max()} nouvelles localisations en une semaine"
        )
...
def validate_virus_spread(virus_name, model, scaler, feature_cols, spread_data):
    ...
    for i in range(10, len(spread_data) - 1):  # Besoin d'historique
        ...
        try:
            ...
            predicted_locations = max(
                0, round(predicted_locations)
        ...
        results.append(
            {
                ...
                "rel_error": abs(predicted_locations - actual_locations)
                / max(actual_locations, 1),
                # Données de contexte
                "cumulative_locations": spread_data.iloc[: i + 1][
                    "new_locations"
                ].sum(),
                "recent_trend": spread_data.iloc[max(0, i - 3) : i + 1][
                    "new_locations"
                ].mean(),
...
def print_validation_metrics(results, virus_name):
    ...
    print(
        f"Pic de propagation: {max_spread_week} nouvelles localisations en une semaine"
    )
    ...
    print(
        f"Semaines de forte propagation (≥5 lieux): {high_spread} ({high_spread / len(df) * 100:.1f}%)"
    )
    print(
        f"Semaines de propagation modérée (2-4 lieux): {medium_spread} ({medium_spread / len(df) * 100:.1f}%)"
    )
    print(
        f"Semaines de faible propagation (<2 lieux): {low_spread} ({low_spread / len(df) * 100:.1f}%)"
    )
...
def show_prediction_details(df, virus_name):
    ...
    try:
        ...
        print(
            f"Nouvelles localisations réelles (moyenne): {display_df['actual'].mean():.1f}"
        )
        print(
            f"Nouvelles localisations prédites (moyenne): {display_df['predicted'].mean():.1f}"
        )
        ...
        if len(display_df) > 5:
            ...
            for month, avg in seasonal_analysis.items():
                ...
                print(
                    f"  {month_name}: {avg:.1f} nouvelles localisations/semaine en moyenne"
                )
...
def save_prediction_details_csv(df, virus_name, title, model_type):
    ...
    try:
        ...
        csv_df["spread_phase"] = csv_df["actual"].apply(
            lambda x: (
                "Forte propagation"
                if x >= 5
                else "Propagation modérée" if x >= 2 else "Faible propagation"
            )
        )
        ...
        final_df.columns = [
            "Date",
            "Nouvelles_Lieux_Prédit",
            "Nouvelles_Lieux_Réel",
            "Erreur_Absolue",
            "Erreur_Relative_%",
            "Phase_Propagation",
            "Total_Cumulé",
...
def save_validation_plots(results, virus_name):
    ...
    try:
        ...
        fig.suptitle(
            f"Validation du Modèle de Propagation Géographique: {virus_name}",
            fontsize=16,
        )
        ...
        plot_path = os.path.join(
            STATIC_DIR,
            f'validation_geographical_{virus_name.lower().replace(" ", "_")}.png',
        )
...
def validate_multiple_viruses(first_cases_df, model, scaler, feature_cols):
    ...
    for virus in viruses:
        ...
        if results:
            ...
            virus_summaries.append(
                {
                    ...
                    "Qualité": (
                        "Excellente"
                        if median_error < 0.20
                        else (
                            "Bonne"
                            if median_error < 0.40
                            else "Correcte" if median_error < 0.70 else "Mauvaise"
                        )
                    ),
                }
            )
            ...
            print(
                f"  ✅ MAE: {mae:.2f}, R²: {r2:.3f}, Propagation moy: {avg_spread:.1f}"
            )
    ...
    if all_results:
        ...
        print(
            f"Propagation globale moyenne: {overall_df['actual'].mean():.1f} nouvelles lieux/semaine"
        )
...
def main():
    ...
    print(
        "🎯 Objectif: Tester la précision des prédictions de propagation vers de nouveaux lieux"
    )
    print("=" * 70)
    ...
    try:
        ...
        if choice == "1":
            ...
            if spread_data is not None:
                results = validate_virus_spread(
                    "COVID", model, scaler, feature_cols, spread_data
                )
                if results:
            ...
            if spread_data is not None:
                results = validate_virus_spread(
                    "Monkeypox", model, scaler, feature_cols, spread_data
                )
                if results:
            ...
            if custom_virus in viruses:
                ...
                if spread_data is not None:
                    results = validate_virus_spread(
                        custom_virus, model, scaler, feature_cols, spread_data
                    )
                    if results:
