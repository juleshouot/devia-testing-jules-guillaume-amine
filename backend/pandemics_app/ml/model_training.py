...
def save_model_metrics_to_db(model_name, model_type, metrics):
    ...
    try:
        ...
        print(
            f"   📊 RMSE: {model_metrics.get('rmse', 0):.4f}, R²: {model_metrics.get('r2_score', 0):.4f}"
        )
...
def prepare_train_test_data_temporal(
    df, target_col, feature_cols, test_size=0.2, random_state=42
):
    """
...
def save_model(model, scaler, feature_cols, model_name, model_type, metrics):
    ...
    joblib.dump(
        model, os.path.join(MODEL_DIR, f"{model_type}_{safe_name}_model.joblib")
    )
    joblib.dump(
        scaler, os.path.join(MODEL_DIR, f"{model_type}_{safe_name}_scaler.joblib")
    )
    ...
    print(
        f"✅ Modèle {model_type} sauvegardé ({model_name}) avec métriques en fichier ET en base."
    )
...
def train_and_evaluate_models(X_train, X_test, y_train, y_test, model_type):
    ...
    for name, model in models.items():
        ...
        print(
            f"  RMSE: {rmse:.4f} | MAE: {mae:.4f} | R²: {r2:.4f} | CV RMSE: {cv_rmse_val:.4f}"
        )
    ...
    best_model_name = min(
        model_metrics_dict, key=lambda x: model_metrics_dict[x]["rmse"]
    )
    print(
...
def train_transmission_model():
    ...
    for df, virus_name in [(covid_df, "COVID"), (monkeypox_df, "Monkeypox")]:
        ...
        for location, group_df in grouped:
            ...
            temp_df["deaths_ma7"] = (
                temp_df["new_deaths"].rolling(window=7, min_periods=1).mean()
            )

            # Décalage pour calculer Rt (taux de reproduction)
            incubation_period = 7  # Estimation moyenne
            temp_df["previous_cases_ma7"] = temp_df["cases_ma7"].shift(
                incubation_period
            )
            ...
            temp_df["cases_growth"] = (
                temp_df["cases_ma7"].pct_change(7).fillna(0).clip(-1, 2)
            )
            temp_df["deaths_growth"] = (
                temp_df["deaths_ma7"].pct_change(7).fillna(0).clip(-1, 2)
            )
            temp_df["days_since_start"] = (
                temp_df["date"] - temp_df["date"].min()
            ).dt.days
    ...
    print(
        f"✅ Données Rt sauvegardées: {os.path.join(DATA_DIR, 'transmission_rt_data.csv')}"
    )
    ...
    X_train, X_test, y_train, y_test, scaler, feature_cols = (
        prepare_train_test_data_temporal(df_final, target_col, feature_cols)
    )
    ...
    save_model(
        models[best_name], scaler, feature_cols, best_name, "transmission", metrics
    )
...
def train_mortality_model():
    ...
    for df, virus_name in [(covid_df, "COVID"), (monkeypox_df, "Monkeypox")]:
        ...
        for location, group_df in grouped:
            ...
            temp_df["treatment_improvement"] = np.exp(
                -0.001 * temp_df["days_since_start"]
            )  # Proxy pour l'amélioration des traitements
            temp_df["healthcare_pressure"] = (
                temp_df["cases_7day"] / temp_df["cases_7day"].max()
            ).fillna(
                0
            )  # Proxy pour la pression sur le système de santé
    ...
    # Sauvegarde des données préparées
    mortality_df = df_final[
        ["date", "location", "virus", "mortality_ratio", "cases_7day", "deaths_7day"]
    ]
    mortality_df.to_csv(os.path.join(DATA_DIR, "mortality_ratio_data.csv"), index=False)
    print(
        f"✅ Données de mortalité sauvegardées: {os.path.join(DATA_DIR, 'mortality_ratio_data.csv')}"
    )
...
def train_geographical_spread_model():
    ...
    final_spread_data.to_csv(
        os.path.join(DATA_DIR, "geographical_spread_data.csv"), index=False
    )
    print(
    ...
    X_train, X_test, y_train, y_test, scaler, feature_cols = (
        prepare_train_test_data_temporal(final_spread_data, target_col, feature_cols)
    )
...
def train_models():
    ...
    try:
        ...
        print(
            f"❌ Erreur lors de l'entraînement du modèle de propagation géographique: {e}"
        )
