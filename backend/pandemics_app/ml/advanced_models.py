...
def label_epidemic_phases(df):
    ...
    for (location, virus), group in df_phases.groupby(["location", "virus"]):
        ...
        temp_df["cases_ma14"] = (
            temp_df["new_cases"].rolling(window=14, min_periods=3).mean()
        )
        ...
        temp_df["epidemic_phase_smoothed"] = (
            temp_df["epidemic_phase"]
    ...
    for phase, count in phase_counts.items():
        ...
        print(
            f"  Phase {phase} ({phase_name}): {count} observations ({count / len(result_df) * 100:.1f}%)"
        )
...
def prepare_phase_prediction_features(df_phases):
    ...
    df_phases["days_since_first_case"] = df_phases.groupby(["location", "virus"])[
        "date"
...
def train_epidemic_phase_model():
    ...
    split_date = df_phases["date"].iloc[
        int(len(df_phases) * 0.8)
    ]  # 80% train, 20% test
    ...
    confusion_matrix_path = os.path.join(
        PLOTS_DIR, "epidemic_phases_confusion_matrix.png"
    )
    ...
    feature_importance_path = os.path.join(
        PLOTS_DIR, "epidemic_phases_feature_importance.png"
    )
    ...
    metadata = {
        ...
        "accuracy": accuracy,
        "phase_names": {1: "Début", 2: "Accélération", 3: "Pic", 4: "Déclin", 5: "Fin"},
    }
...
def simplified_seasonal_analysis(df):
    ...
    for virus, virus_df in df.groupby("virus"):
        ...
        monthly_avg = (
            virus_df.groupby("month")
            .agg({"new_cases": "mean", "month_name": "first"})
            .reset_index()
        )
        ...
        if avg_all_months > 0:
            monthly_avg["seasonal_effect"] = (
                monthly_avg["new_cases"] / avg_all_months
            ) - 1
        else:
        ...
        for location in top_regions:
            ...
            if avg_all_months_location > 0:
                location_monthly["seasonal_effect"] = (
                    location_monthly["new_cases"] / avg_all_months_location
                ) - 1
            else:
...
def create_simplified_seasonal_model(seasonal_data):
    ...
    for virus in seasonal_data["virus"].unique():
        virus_data = seasonal_data[
            (seasonal_data["virus"] == virus) & (~seasonal_data["location"].notna())
        ].copy()
        ...
        virus_data["seasonal_risk"] = (virus_data["seasonal_effect"] - min_effect) / (
            max_effect - min_effect
        )

        # Identifier les mois à haut risque (>0.7)
        high_risk_months = virus_data[virus_data["seasonal_risk"] > 0.7][
            "month"
        ].tolist()

        # Créer le modèle
        seasonal_models[virus] = {
            "monthly_risk": virus_data[["month", "seasonal_risk"]]
            .set_index("month")["seasonal_risk"]
            .to_dict(),
        ...
        location_models = {}
        for location in (
            seasonal_data[seasonal_data["virus"] == virus]["location"].dropna().unique()
        ):
            loc_data = seasonal_data[
                (seasonal_data["virus"] == virus)
                & (seasonal_data["location"] == location)
            ].copy()
            ...
            loc_data["seasonal_risk"] = (loc_data["seasonal_effect"] - min_loc) / (
                max_loc - min_loc
            )
            high_risk_loc = loc_data[loc_data["seasonal_risk"] > 0.7]["month"].tolist()

            location_models[location] = {
                "monthly_risk": loc_data[["month", "seasonal_risk"]]
                .set_index("month")["seasonal_risk"]
                .to_dict(),
