...
def load_transmission_model():
    ...
    try:
        ...
        model_files = [
            ...
            if "transmission" in f and f.endswith("_model.joblib")
        ]
...
def calculate_actual_rt(df, current_idx):
    ...
    try:
        ...
        if current_idx >= 6:
            current_cases = df.iloc[current_idx - 6 : current_idx + 1][
                "new_cases"
            ].mean()
        else:
            return None
...
def validate_single_location(location, model, scaler, feature_cols):
    ...
    for i in range(
        30, len(df) - 14
    ):  # Besoin d'historique et de données futures pour validation
        ...
        try:
            ...
            if i < 35:  # Premières prédictions seulement
                print(
                    f"DEBUG {i}: Features sample: cases_ma7={features_dict.get('cases_ma7', 0):.2f}, "
        ...
        if actual_rt is not None:
            ...
            if i < 35:
                print(
                    f"DEBUG {i}: actual_rt={actual_rt:.3f}, predicted_rt={predicted_rt:.3f}, "
    ...
    print(
        f"✅ {len(results)} prédictions générées pour prédire Rt 7-14 jours à l'avance"
    )
    return results
...
def print_validation_metrics(results, location):
    ...
    print(
        f"Périodes épidémiques futures (Rt > 1): {epidemic_periods} ({epidemic_periods / len(df) * 100:.1f}%)"
    )
    print(
        f"Périodes contrôlées futures (Rt ≤ 1): {controlled_periods} ({controlled_periods / len(df) * 100:.1f}%)"
    )
...
def show_prediction_details(df, location):
    ...
    try:
        ...
        print("=" * 140)
        print(
            f"{'Date':<12} {'Rt Prédit':<10} {'Rt Futur':<10} {'Erreur':<8} {'Erreur%':<8} {'État Futur':<12} "
        ...
        for _, row in display_df.iterrows():
            ...
            print(
                f"{date_str:<12} {predicted_rt:<10} {actual_rt:<10} {error_abs:<8} {error_rel:<8} "
...
def save_prediction_details_csv(df, location, title, model_type):
    ...
    try:
        ...
        csv_df["future_epidemic_state"] = csv_df["actual"].apply(
            lambda x: (
                "Forte croissance"
                if x > 1.5
                else "Croissance" if x > 1.0 else "Stable" if x > 0.8 else "Déclin"
            )
        )

        csv_df["prediction_quality"] = csv_df["rel_error"].apply(
            lambda x: (
                "Excellente"
                if x < 0.15
...
def save_validation_plots(results, location):
    ...
    try:
        ...
        axes[0, 1].plot(
            df["date"], df["actual"], label="Rt Futur Réel", alpha=0.8, linewidth=2
        )
        axes[0, 1].plot(
            df["date"], df["predicted"], label="Rt Futur Prédit", alpha=0.8, linewidth=2
        )
        axes[0, 1].axhline(
            y=1, color="red", linestyle="--", alpha=0.7, label="Seuil épidémique (Rt=1)"
        )
        ...
        plot_path = os.path.join(
            STATIC_DIR,
            f'validation_transmission_fixed_{location.lower().replace(" ", "_")}.png',
        )
...
def main():
    ...
    print(
        "🔧 Correction: Évite le data leakage en utilisant vraiment des données futures"
    )
    print("=" * 80)
    ...
    print(
        f"🔧 Cette validation évite le data leakage et teste vraiment la prédiction future!"
    )
