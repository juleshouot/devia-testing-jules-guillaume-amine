...
from .models import Location, Virus, Prediction, GeographicalSpreadPrediction, ModelMetrics
...
class PredictorTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        model_dir = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'pandemics_app', 'ml', 'models')
    ...
    def test_predict_geographical_spread(self):
        ...
        geographical_spread = self.predictor.predict_geographical_spread(self.spread_data)
    ...
    def test_predict_combined(self):
        ...
        if 'transmission_rate' in predictions:
            self.assertIsInstance(predictions['transmission_rate'], (int, float, np.number))
        
        if 'mortality_rate' in predictions:
            self.assertIsInstance(predictions['mortality_rate'], (int, float, np.number))
    ...
    def test_generate_forecast(self):
        ...
        forecasts = self.predictor.generate_forecast(self.location_data, self.spread_data, weeks=4)
